...
<!-- original: none | Авторская сводная схема JAXP; оригинальная фигура на вводной странице Oracle отсутствует -->
```mermaid
flowchart TB
    XML["XML-данные"]
...
<!-- original: assets/17-jaxp/jaxp-sax-api.gif | Рисунок 1-1: API SAX — SAXParserFactory, SAXParser, SAXReader и интерфейсы-обработчики -->
```mermaid
flowchart LR
...
<!-- original: assets/17-jaxp/jaxp-dom-api.gif | API DOM — DocumentBuilderFactory создаёт DocumentBuilder, который разбирает XML в дерево Document -->
```mermaid
flowchart LR
...
<!-- original: assets/17-jaxp/jaxp-xslt-api.gif | API XSLT — TransformerFactory создаёт Transformer, принимающий Source и выдающий Result -->
```mermaid
flowchart LR
