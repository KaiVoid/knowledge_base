...
<!-- original: none | Авторская схема иерархии данных MIDI (Sequence → Track → MidiEvent); страница Oracle не содержит соответствующего рисунка -->
```mermaid
flowchart TB
...
<!-- original: none | Авторская схема потока данных MIDI-устройств (порт → секвенсор → синтезатор/выходной порт); страница Oracle не содержит соответствующего рисунка -->
```mermaid
flowchart LR
